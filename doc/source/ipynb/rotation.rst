...
A patch is a square window (*s_p\times s_p*) from the image. The patches are overlapped one by one. The distance of the center between two neighbor patches is the patch steps ($n_s$). The patch number is 
